...
function createAgendamentoCard(agendamento) {
    ...
    const compartilhamentoRecebido = agendamento.compartilhadoPor ? 
        `<div class="postit-row postit-observacao postit-shared-received">
            <span><b>Compartilhado por:</b> ${agendamento.compartilhadoPor}</span>
            ${agendamento.shareMessage ? `<br><span class="share-message"><b>Mensagem:</b> ${agendamento.shareMessage}</span>` : ''}
        </div>` : '';
        ...
        <div class="agendamento-card ${statusClass} ${postitStyle} ${shareClass}" data-id="${agendamento.id}" data-shared="${isShared}">
            ...
            <div class="postit-footer">
                ${agendamento.status !== 'Concluído' && agendamento.status !== 'Cancelado' ? `
                    ...
                    ` : `
                        <button class="postit-btn editar-btn disabled" disabled title="Limite de edições atingido (${MAX_EDIT_COUNT}/${MAX_EDIT_COUNT})">
                            <i class="fa-solid fa-edit"></i> Editar
                        </button>
                    `}
                ` : ''}
...
function openEditModal(id) {
    ...
    if (editCount >= MAX_EDIT_COUNT) {
        showToast(`Limite de edições atingido (${MAX_EDIT_COUNT}/${MAX_EDIT_COUNT})`, 'warning');
        return;
    }
...
function handleWebSocketAgendamentoShared(data) {
    ...
    if (window.TTSNotifications) {
        window.TTSNotifications.agendamentoRecebido(
            agendamento.nomeCliente || agendamento.cliente, 
    ...
    // Mostrar toast
            showToast(`Agendamento compartilhado por ${fromUser.displayName}`, 'success');
    ...
    if (window.notificationSystem) {
        window.notificationSystem.createNotification({
            ...
            message: `${fromUser.displayName} compartilhou um agendamento com você: ${agendamento.nomeCliente || agendamento.cliente}`,
            data: {
            ...
            actions: [
                {
                    ...
                    style: 'primary',
                    callback: (data) => {
                        console.log('[DEBUG] Ação ver agendamento compartilhado clicada:', data);
                        // Aqui você pode implementar a lógica para mostrar o agendamento
                    }
                },
                {
                    id: 'accept',
                    ...
                    style: 'success',
                    callback: (data) => {
                        console.log('[DEBUG] Agendamento aceito:', data);
...
function handleWebSocketNotificationReceived(data) {
    ...
    console.log('[DEBUG] Notificação recebida:', notification);
    
    // Mostrar toast
    showToast(notification.message, 'info');
    
    // Se a notificação tem TTS habilitado, reproduzir
    if (notification.tts && notification.tts.enabled && window.TTSNotifications) {
        window.TTSNotifications.speak(notification.tts.text, notification.tts.priority);
    }
    
    // Criar notificação visual se o sistema estiver disponível
    if (window.notificationSystem) {
        window.notificationSystem.createNotification({
            type: notification.type === 'share' ? 'info' : notification.type,
            ...
            actions: [
                {
                    ...
                    style: 'primary',
                    callback: (data) => {
                        console.log('[DEBUG] Ação ver detalhes clicada:', data);
                    }
                },
                {
                    id: 'dismiss',
                    ...
                    style: 'secondary',
                    callback: (data) => {
                        console.log('[DEBUG] Notificação fechada');
                    }
                }
            ]
        });
...
// Manipular evento TTS via WebSocket
function handleWebSocketTTSSpeak(data) {
    const { text, priority, type } = data;
    
    console.log('[DEBUG] Evento TTS recebido:', { text, priority, type });
    
    // Reproduzir TTS se o sistema estiver disponível
    if (window.TTSNotifications) {
        if (type === 'share_received') {
            // Extrair informações do texto para notificação específica
            const match = text.match(/Você recebeu um agendamento de (.+?) de (.+?)(?:\. Mensagem: (.+))?$/);
            if (match) {
                const [, nomeCliente, compartilhadoPor, mensagem] = match;
                window.TTSNotifications.agendamentoRecebido(nomeCliente, compartilhadoPor, mensagem);
            } else {
                window.TTSNotifications.speak(text, priority);
            }
        } else {
            window.TTSNotifications.speak(text, priority);
        }
