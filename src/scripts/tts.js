...
class TTSManager {
    ...
    speakAgendamentoAtrasado(nomeCliente, minutosAtraso, nomeAtendente = '') {
        ...
        setTimeout(() => {
            ...
            if (nomeAtendente && nomeAtendente.trim() !== '') {
...
window.TTSNotifications = {
    ...
    agendamentoCompartilhado: (nomeCliente, compartilhadoPor, mensagem = '') => {
        ...
        const textoCompleto = mensagem 
            ? `Agendamento de ${nomeCliente} foi compartilhado por ${compartilhadoPor}. Mensagem: ${mensagem}`
            : `Agendamento de ${nomeCliente} foi compartilhado por ${compartilhadoPor}`;
        
        ttsManager.speak(textoCompleto, { priority: 1 });
        ...
        if (window.notificationSystem) {
            window.notificationSystem.createNotification({
                ...
                message: `${compartilhadoPor} compartilhou um agendamento com você: ${nomeCliente}`,
                data: { 
                    nomeCliente, 
                    compartilhadoPor, 
                    mensagem,
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        style: 'secondary',
                        callback: (data) => {
                            console.log('[DEBUG] Notificação de compartilhamento fechada');
                        }
                    }
                ]
            });
        ...
        // Mostrar toast
        if (window.showToast) {
            window.showToast(`Agendamento compartilhado por ${compartilhadoPor}`, 'info');
        }
        
        console.log('[DEBUG] Notificação de agendamento compartilhado enviada');
    },
    
    agendamentoRecebido: (nomeCliente, compartilhadoPor, mensagem = '') => {
        console.log('[DEBUG] TTSNotifications.agendamentoRecebido chamado:', { nomeCliente, compartilhadoPor, mensagem });
        
        const textoCompleto = `Você recebeu um novo agendamento: ${nomeCliente}`;
        
        ttsManager.speak(textoCompleto, { priority: 2 });
        
        // Criar notificação visual urgente
        if (window.notificationSystem) {
            window.notificationSystem.createNotification({
                type: 'urgent',
                ...
                actions: [
                    {
                        ...
                        style: 'primary',
                        callback: (data) => {
                            console.log('[DEBUG] Ação ver agendamento recebido clicada:', data);
                            // Aqui você pode implementar a lógica para mostrar o agendamento
                        }
                    },
                    {
                        ...
                        callback: (data) => {
                            ...
                            if (window.showToast) {
                                window.showToast('Agendamento aceito com sucesso!', 'success');
                            }
                        }
                    }
                ]
        ...
        // Mostrar toast
        if (window.showToast) {
            window.showToast(`Novo agendamento recebido de ${compartilhadoPor}`, 'success');
        }
        
        console.log('[DEBUG] Notificação de agendamento recebido enviada');
    },
