...
class Database {
    ...
    async getSyncLog(since = null) {
        ...
        `;
        const params = since ? [since.toISOString()] : [new Date(0).toISOString()];
        return await this.all(sql, params);
    }

    /**
     * Buscar agendamentos atualizados desde uma data específica
     */
    async getAppointmentsUpdatedSince(since) {
        const sql = `
            SELECT * FROM appointments 
            WHERE updated_at >= ? 
    ...
    /**
     * Buscar notificações criadas desde uma data específica
     */
    ...
    /**
     * Buscar mudanças de status desde uma data específica
     */
    async getStatusChangesSince(since) {
        const sql = `
            SELECT * FROM sync_log 
